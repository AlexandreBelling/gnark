...
func Solve(r1cs *cs.R1CS, witness bn254witness.Witness, opt backend.ProverOption) (wires, a, b, c []fr.Element, err error) {
	...
	if wires, err = r1cs.Solve(witness, a, b, c, opt); err != nil {
		if !opt.Force {
			return wires, a, b, c, err
		} else {
			// we need to fill wires with random values else multi exps don't do much
			var r fr.Element
			_, _ = r.SetRandom()
			for i := r1cs.NbPublicVariables + r1cs.NbSecretVariables; i < len(wires); i++ {
				wires[i] = r
				r.Double(&r)
			}
		}
...
func ComputeProof(r1cs *cs.R1CS, pk *ProvingKey, a, b, c, wireValues []fr.Element) (*Proof, error) {
